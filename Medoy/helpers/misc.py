...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("Medoy").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("Medoy").info(f"Invalid Git Command")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("Medoy").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    desc = "Group Log untuk Medoy-UserBot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n✨ Powered By ~ @Lunatic0de ✨"
    try:
        gruplog = await client.create_supergroup("Medoy Userbot Logger", desc)
        if await in_heroku():
